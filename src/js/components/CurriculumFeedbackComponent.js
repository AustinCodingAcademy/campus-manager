...
module.exports = React.createBackboneClass({
  ...
  componentDidMount() {
    window.addEventListener("message", function(e) {
      if (typeof e.data === 'string' && e.data.indexOf('curriculum-feedback-iframe') > -1) {
        document.getElementById('curriculum-feedback-iframe').className = 'hidden';
  ...
  render() {
    return (
      <Modal show={this.props.show} onHide={this.props.onHide}>
        ...
        <Modal.Body>
          <div className="iframe-wrapper" id="curriculum-feedback-iframe">
            <div className="h-iframe">
              <div className="ratio" id="ratio" />
              <iframe src="/curriculum-feedback" frameBorder="0"></iframe>
            </div>
          </div>
